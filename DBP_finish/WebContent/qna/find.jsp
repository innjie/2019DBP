...
<a href="<c:url value='/qna/view'>
		...
		<c:forEach var="page" begin="1" end="${pageSize}" step="1" varStatus="varStatus">
			...
			<a href="<c:url value='/qna/find'>
				<c:param name='currentPage' value="${page}"/>
				<c:param name='findContent' value="${question}"/>
				</c:url>">
			${page}</a>
			</c:if>
			<c:if test="${currentPage == page}">
		...
		<br><br>  
			<input type="button" class="btn btn-primary" value="메인으로" onClick="qnaList(<c:url value='/qna/list' />)">
	</form>
	</div>
      </div>
    </div>
